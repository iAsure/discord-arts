function parseImg(imgString) {
  if (!imgString || typeof imgString !== "string") {
    throw new Error(
      `Discord Arts | Invalid custom badge ('${
        imgString || undefined
  ...
  if (!imgCheck)
    throw new Error(
      `Discord Arts | Invalid customBackground ('${
        imgString || undefined
      }') must be an image file 'png | jpg | gif'`
    );
...
function parseHex(hexString) {
  ...
  if (!hexRegex.test(hexString))
    throw new Error(
      `Discord Arts | Invalid Hex Code ('${hexString || undefined}')`
    );
...
function isString(param, type) {
  if (typeof param !== "string")
    throw new Error(
      `Discord Arts | Invalid ${type} (${param || undefined}), must be a string`
    );
...
function isNumber(param, type) {
  if (isNaN(param))
    throw new Error(
      `Discord Arts | Invalid ${type} (${param || undefined}), must be a number`
    );
